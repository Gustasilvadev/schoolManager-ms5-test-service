...
const createGrade = async (req, res, next) => {
  try {
    const newGrade = await gradeService.createGrade(req.body, req.user, req.headers.authorization);
...
const bulkCreateGrades = async (req, res, next) => {
  try {
    ...
    const result = await gradeService.bulkCreateGrades(grades, req.user, req.headers.authorization);
...
const getGradesByStudent = async (req, res, next) => {
  try {
    ...
    const grades = await gradeService.getGradesByStudent(parseInt(studentId), req.headers.authorization);
    return res.status(HTTP_STATUS.OK).json(grades);
  } catch (error) {
...
const updateGrade = async (req, res, next) => {
  try {
    const { id } = req.params;
    const updated = await gradeService.updateGrade(parseInt(id), req.body, req.user, req.headers.authorization);
...
const deleteGrade = async (req, res, next) => {
  try {
    const { id } = req.params;
    await gradeService.deleteGrade(parseInt(id), req.user, req.headers.authorization);
